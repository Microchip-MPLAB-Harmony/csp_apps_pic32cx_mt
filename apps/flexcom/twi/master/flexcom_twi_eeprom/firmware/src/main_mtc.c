    ...
    while(1)
    {
        ...
        switch (state)
        {
            case APP_STATE_EEPROM_STATUS_VERIFY:
                ...
                FLEXCOM4_TWI_CallbackRegister( APP_TWICallback, (uintptr_t)&transferStatus );

               /* Verify if EEPROM is ready to accept new requests */
                transferStatus = APP_TRANSFER_STATUS_IN_PROGRESS;
                FLEXCOM4_TWI_Write(APP_AT24MAC_DEVICE_ADDR, &ackData, APP_ACK_DATA_LENGTH);
            ...
            case APP_STATE_EEPROM_WRITE:
                ...
                if (transferStatus == APP_TRANSFER_STATUS_SUCCESS)
                {
                    ...
                    transferStatus = APP_TRANSFER_STATUS_IN_PROGRESS;
                    FLEXCOM4_TWI_Write(APP_AT24MAC_DEVICE_ADDR, &testTxData[0], APP_TRANSMIT_DATA_LENGTH);
            ...
            case APP_STATE_EEPROM_READ:
                ...
                FLEXCOM4_TWI_WriteRead(APP_AT24MAC_DEVICE_ADDR, &testTxData[0], APP_RECEIVE_DUMMY_WRITE_LENGTH,  &testRxData[0], APP_RECEIVE_DATA_LENGTH);
